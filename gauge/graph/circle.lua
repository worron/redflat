...
local function default_style()
	local style = {
		...
		iradius      = 6,
		step         = 0.02,
		color        = { main = "#b1222b", gray = "#575757", icon = "#a0a0a0", mute = "#fbe232" }
	}
...
function cirmon.new(style)
	...
	widg._data = { color = style.color.icon, level = 0, alert = false, mute = false }
	...
	function widg:draw(_, cr, width, height)
		...
		for i = 1, 2 do
			if self._data.mute then
				cr:set_source(color(i > 1 and style.color.mute or style.color.gray))
			else
	...
	-- specialized function for audio only
	function widg:set_mute(mute)
		self._data.mute = mute
		self._data.color = mute and style.color.gray or style.color.icon
		self:emit_signal("widget::redraw_needed")
	end
